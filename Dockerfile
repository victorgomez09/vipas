...
RUN CGO_ENABLED=0 GOOS=${TARGETOS} GOARCH=${TARGETARCH} go build -trimpath \
    -ldflags "-s -w -X github.com/victorgomez09/vipas/apps/api/internal/version.Version=${VERSION}" \
    -o /usr/local/bin/vipas-api ./cmd/server && \
...
RUN apk add --no-cache ca-certificates tzdata curl docker-cli docker-cli-compose \
    && addgroup -S vipas && adduser -S vipas -G vipas

COPY --from=api-builder /usr/local/bin/vipas-api /usr/local/bin/vipas-api
