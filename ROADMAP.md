...
- [x]  **2.2.1** Añadir dependencia en `go.mod`:
 ...
 - [x] **2.2.2** Crear `apps/api/internal/orchestrator/k3s/httproute.go`:
  ...
  - [x] `UpdateHTTPRoute()` — actualiza puerto y config TLS si cambia
  - [x] `DeleteHTTPRoute()` — elimina el `HTTPRoute`
  - [x] `SyncHTTPRoutePorts()` — equivalente al actual `SyncIngressPorts`
  - [x] `GetHTTPRouteStatus()` — lee `.status.parents[].conditions` (busca `Accepted` y `ResolvedRefs`)
  - [x] `GetCertExpiry()` — lee el Secret de cert-manager del namespace de la app
  - [x] `EnsurePanelHTTPRoute()` — equivalente al actual `EnsurePanelIngress`, apunta al Service `vipas`

 - [x] **2.2.3** Crear `apps/api/internal/orchestrator/k3s/gateway.go`:
  - [x] `EnsureGateway()` — crea/actualiza el `GatewayClass` y el `Gateway` central si no existen (idempotente)

### 2.3 — Eliminar código Traefik

 - [x] **2.3.1** Eliminar `apps/api/internal/orchestrator/k3s/ingress.go` (parte)
 - [x] **2.3.2** Eliminar `apps/api/internal/orchestrator/k3s/traefik.go` (parte)
 - [x] **2.3.3** Eliminar el middleware `EnsureRedirectHTTPSMiddleware` (parte — ahora lo hace `HTTPRequestRedirectFilter` nativo de Gateway API)

### 2.4 — Modelo de datos

 - [x] **2.4.1** En `model/domain.go`:
  - Renombrar `IngressReady` → `RouteReady` (mantener columna en BD con `bun:"ingress_ready"` alias temporal)
  - [x] Eliminar `CertSecret` (cert-manager gestiona los Secrets directamente)

- [x] **2.4.2** Crear migración de BD:
  - Renombrar columna `ingress_ready` → `route_ready` en tabla `domains`
  - Eliminar columna `cert_secret` (migraciones añadidas)
 - [x] **2.4.3** En `model/setting.go`, añadir constante:
  ```go
...
### 2.6 — API HTTP
 ...
 - [x] **2.6.1** Listar y revisar todos los handlers en `apps/api/internal/api/v1/` que referencien Traefik o Ingress
 - [x] **2.6.2** Eliminar endpoints de Traefik (`GET/PUT /api/v1/settings/traefik`) — devolvían `404 NotFound` y la UI fue adaptada parcialmente
...
## Fase 4 — Nodos dedicados al Gateway (prod)
...
## Fase 6 — Cilium: red avanzada (prod)
...
## Fase 7 — external-dns: DNS automático
...
## Fase 8 — Storage distribuido (prod)

> En dev se usa `local-path`. En prod se necesita storage replicado para soportar múltiples nodos.

- [ ] **8.1** Elegir:
  - [ ] **8.1.a** **Longhorn** — más sencillo, UI incluida, replicación automática
  - [ ] **8.1.b** **Rook/Ceph** — mayor rendimiento para I/O intensivo
- [ ] **8.2** Instalar Longhorn:
  ```bash
  helm install longhorn longhorn/longhorn \
    -n longhorn-system --create-namespace \
    --set defaultSettings.defaultReplicaCount=3
  ```
- [ ] **8.3** Hacer `longhorn` la `StorageClass` por defecto
- [ ] **8.4** Actualizar `storage.go`: crear PVCs con `storageClassName: longhorn` en prod; `local-path` en single-node
- [ ] **8.5** Integrar snapshots de Longhorn con `system_backup_service.go` (backup a S3)
- [ ] **8.6** Añadir UI de volúmenes en el panel (estado, replicación, snapshots)

---

## Fase 9 — Base de datos HA: CloudNativePG (prod)

- [ ] **9.1** Instalar el operador **CloudNativePG**:
  ```bash
  helm install cnpg cloudnative-pg/cloudnative-pg -n cnpg-system --create-namespace
  ```
- [ ] **9.2** En `model/database.go`: añadir campo `ha_mode bool` y `replicas int`
...
- [ ] **9.4** Adaptar `GetDatabaseCredentials()` para extraer credenciales del Secret de CloudNativePG
- [ ] **9.5** Añadir soporte para connection pooling con PgBouncer (incluido en CloudNativePG)
- [ ] **9.6** Integrar backups de CloudNativePG con `system_backup_service.go` (S3)
- [ ] **9.7** Migración de BD de Vipas para los nuevos campos en `managed_databases`

---

## Fase 10 — Observabilidad

- [ ] **10.1** Instalar **kube-prometheus-stack**:
  ```bash
  helm install kps prometheus-community/kube-prometheus-stack \
    -n monitoring --create-namespace
  ```
- [ ] **10.2** Configurar scraping de métricas de Envoy Gateway (endpoint `/stats/prometheus`)
- [ ] **10.3** Configurar scraping de métricas de Cilium y Hubble
- [ ] **10.4** Dashboards de Grafana:
...
- [ ] **10.5** Reglas de Alertmanager:
  - Gateway sin endpoints disponibles
  - Certificado TLS con < 7 días para expirar
  - Nodo gateway CPU > 80%
  - Quorum etcd roto (< 2 de 3 nodos)
- [ ] **10.6** Integrar alertas con `notification_service.go` (email, Slack, webhook)
- [ ] **10.7** Integrar métricas de Envoy en `metrics_collector.go` — requests/s y latencia por dominio
- [ ] **10.8** Exponer Grafana desde el panel de Vipas

---

## Fase 11 — Panel de administración (UI)

- [ ] **11.1** Sección "Dominios":
  - `IngressReady` → `RouteReady`
  - Mostrar condiciones del `HTTPRoute` (`Accepted`, `ResolvedRefs`)
  - Mostrar IP asignada por el LB / NodePort
- [ ] **11.2** Sección "Gateway":
  - Estado del `Gateway` y sus listeners (`:80` / `:443`)
  - Número de `HTTPRoutes` activos
  - Métricas de tráfico agregadas
- [ ] **11.3** Sección "Nodos Gateway" separada de "Workers" en la vista del cluster
- [ ] **11.4** Settings: reemplazar configuración de Traefik por configuración de Envoy Gateway:
  - Concurrencia (worker threads)
  - Rate limiting global (`BackendTrafficPolicy`)
  - Timeouts
- [ ] **11.5** Sección "Certificados TLS":
...
- [ ] **12.1** **RBAC** granular: revisar que los `ServiceAccount` tienen permisos mínimos
- [ ] **12.2** **Pod Security Admission** con política `restricted` por namespace de usuario
- [ ] **12.3** `CiliumNetworkPolicy` egress por defecto: denegar todo excepto DNS y gateway
- [ ] **12.4** **Secrets en Vault** (HashiCorp Vault + External Secrets Operator) — `EnsureSecret()` delega en ESO
- [ ] **12.5** **Audit Logging** del API Server → sistema de logs centralizado
- [ ] **12.6** **seccomp** + **AppArmor** profiles en pods de las apps desplegadas
- [ ] **12.7** Escaneo de imágenes con Trivy integrado en `build_service.go`
- [ ] **12.8** Rotación automática de `HostKeyFingerprint` en `ServerNode` tras reinstalación

---

## Fase 13 — Documentación y runbooks

- [ ] **13.1** `README.md`: onboarding completo (single node + multi-node)
- [ ] **13.2** `CONTRIBUTING.md`: stack de desarrollo local con el nuevo setup
- [ ] **13.3** Runbook: fallo de un nodo gateway
- [ ] **13.4** Runbook: pérdida de quorum etcd
- [ ] **13.5** Runbook: renovación manual de certificados TLS
- [ ] **13.6** Runbook: escalado horizontal de nodos worker
- [ ] **13.7** Diagrama de arquitectura final (draw.io XML en `deploy/docs/`)
...
Fase 1  Stack base (K3s + Cilium + Envoy Gateway + cert-manager)
  └─► Fase 2  Código Go: IngressManager → GatewayManager
        ...
        ├─► Fase 7  external-dns
        ├─► Fase 8  Storage Longhorn
        │     └─► Fase 9  CloudNativePG
        ├─► Fase 10  Observabilidad
        └─► Fase 11  Seguridad

Fases 1+2 ──► Fase 12  UI
Fase 3   ──► Fase 13  Documentación y runbooks
```
