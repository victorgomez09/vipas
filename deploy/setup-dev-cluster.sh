...
install_metallb() {
  echo "[info] installing MetalLB (layer2)"
  install_helm
  METALLB_VERSION=${METALLB_VERSION:-v0.15.0}
  ...
  echo "[info] installing/upgrading MetalLB Helm chart (waiting for rollout)"
  if ! helm upgrade --install metallb metallb/metallb -n ${NS} --create-namespace --version "${METALLB_VERSION}" --wait --timeout 300s >/dev/null 2>&1; then
    echo "[warn] metallb helm install/upgrade returned non-zero"
    helm -n ${NS} status metallb || true
  fi

  # Default pool can be overridden with METALLB_POOL env var (e.g. 172.26.31.240-172.26.31.250)
  METALLB_POOL=${METALLB_POOL:-172.26.31.240-172.26.31.250}
  NODE_IP=$(sh -c "${KUBECTL_CMD} get nodes -o jsonpath='{.items[0].status.addresses[?(@.type==\"InternalIP\")].address}'" 2>/dev/null || echo "")
  echo "[info] MetalLB pool: ${METALLB_POOL} (node IP: ${NODE_IP})"
...
EOF
  ...
  echo "[info] waiting for MetalLB components (short wait)"
  ${KUBECTL_CMD} -n ${NS} get pods -o wide || true
  ${KUBECTL_CMD} -n ${NS} wait --for=condition=available deployment --all --timeout=120s || true
}
...
configure_local_dns() {
  ...
  # ---- 1. Determine Envoy Gateway VIP from MetalLB ----
  GW_IP=""
  ...
  if [ -z "$GW_IP" ]; then
    # Fallback: use the first IP in the MetalLB pool
    GW_IP=$(echo "${METALLB_POOL:-172.26.31.240-172.26.31.250}" | cut -d- -f1)
...
# Resolve *.localhost to the Envoy Gateway VIP (MetalLB)
